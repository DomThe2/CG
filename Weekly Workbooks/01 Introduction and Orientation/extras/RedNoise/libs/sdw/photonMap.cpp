	...
	if (recursionDepth <= 0) return;
		...
		if (face.opacity < 1.0f) { //Material transparent
			...
			if (glm::length(refracted) < 1e-6f) { //TIR
				...
				tracePhoton(model, output, intersection.intersectionPoint, R, power, recursionDepth-1); // + R*0.05f
			} else { //Refraction
				refracted = glm::normalize(refracted);
				glm::vec3 newPower = power * face.transmission;
				tracePhoton(model, output, intersection.intersectionPoint, refracted, newPower, recursionDepth-1); // + refracted*0.001f
			}
		} else {
			if (face.mirror) { //Mirror, reflect all
				...
				tracePhoton(model, output, intersection.intersectionPoint, R, newPower, recursionDepth-1); // + normal*0.05f
				...
				if (roulette) {
					...
					tracePhoton(model, output, intersection.intersectionPoint, direction, newPower/prob, recursionDepth-1); //+ normal*0.05f
				}
			}
		}
	}
